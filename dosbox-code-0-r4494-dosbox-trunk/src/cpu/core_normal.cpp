...
// Added for instruction trace logging
...
#endif
		// ===== AI TEACHING MODIFICATION START =====
		// Save CPU state before instruction execution for trace comparison
		if (TraceLogger::IsEnabled()) {
			TraceLogger::SaveState();
		}

		// Save instruction address for trace recording
		PhysPt instruction_address = core.cseip;
		// ===== AI TEACHING MODIFICATION END =====
...
restart_opcode:
		...
		// ===== AI TEACHING MODIFICATION START =====
		// Record instruction execution after it completes
		if (TraceLogger::IsEnabled()) {
			// TODO: Get actual instruction string (would need disassembler)
			char inst_str[64];
			sprintf(inst_str, "INST_%04X", instruction_address & 0xFFFF);
			TraceLogger::RecordInstruction(inst_str, instruction_address);
		}
		// ===== AI TEACHING MODIFICATION END =====
	}
	FillFlags();
