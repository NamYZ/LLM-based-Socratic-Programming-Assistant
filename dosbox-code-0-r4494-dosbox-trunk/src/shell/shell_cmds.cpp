...
#include "support.h"
#include "control.h"
// ===== AI TEACHING MODIFICATION START =====
// Added for trace logging commands
#include "trace_logger.h"
// ===== AI TEACHING MODIFICATION END =====
#include <algorithm>
#include <cstring>
#include <cctype>
#include <cstdlib>
...
// ===== AI TEACHING MODIFICATION START =====
// Added trace logging commands for instruction execution recording
{	"TRACE_ENABLE",	0,		&DOS_Shell::CMD_TRACE_ENABLE,	"Enable instruction trace logging"},
{	"TRACE_DISABLE",0,		&DOS_Shell::CMD_TRACE_DISABLE,	"Disable instruction trace logging"},
// ===== AI TEACHING MODIFICATION END =====
...
// ===== AI TEACHING MODIFICATION START =====
// Command to enable instruction trace logging
void DOS_Shell::CMD_TRACE_ENABLE(char *args) {
	if (!args || strlen(args) == 0) {
		WriteOut("Usage: TRACE_ENABLE <output_file>\n");
		return;
	}

	// Remove leading spaces
	char* output_file = args;
	while (*output_file == ' ') output_file++;

	TraceLogger::Enable(output_file);
	WriteOut("Trace logging enabled. Output: %s\n", output_file);
}

// Command to disable instruction trace logging and save to file
void DOS_Shell::CMD_TRACE_DISABLE(char *args) {
	if (!TraceLogger::IsEnabled()) {
		WriteOut("Trace logging is not enabled.\n");
		return;
	}

	TraceLogger::Disable();
	WriteOut("Trace logging disabled. Output saved.\n");
}
// ===== AI TEACHING MODIFICATION END =====
